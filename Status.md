...
2. **Haiku gate check** (on by default, `GATE_CHECK=true`, ~2-3s latency) — independent Haiku one-shot that sees ONLY `groundRules.md` + the command. No user context (resistant to conversational prompt injection). Returns ALLOW/DENY with reason.
...
| `RESULT_HMAC_KEY` | Yes | — (auto-generated by `install.sh`) |
